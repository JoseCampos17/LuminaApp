...
export function displayRecurring() {
  ...
  export function formatCurrency(amount: number): string {
    ...
    // This amount is already in COP/Local currency
    return formatAmountDirectly(amount, financeState.currency, financeState.currencies);
  }
  ...
  export async function loadData() {
    try {
      ...
      await invoke("ensure_salary_generated", {
        year: financeState.selectedYear,
        month: financeState.selectedMonth + 1
      });
