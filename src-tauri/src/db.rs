...
pub fn init(app_handle: &AppHandle) -> Result<Connection, Box<dyn std::error::Error>> {
    ...
    // --- INJECT COHERENT TEST DATA FOR SCREENSHOTS ---
    let tx_count: i64 = conn.query_row("SELECT COUNT(*) FROM view_transactions", [], |r| r.get(0))?;
    if tx_count == 0 {
        let now_secs = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        let today = format_date_from_secs(now_secs);
        let yesterday = format_date_from_secs(now_secs - 86400);
        let three_days_ago = format_date_from_secs(now_secs - 86400 * 3);
        let five_days_ago = format_date_from_secs(now_secs - 86400 * 5);

        // 1. Set a standard salary (2.5 million COP per Quincena)
        conn.execute("UPDATE settings SET value = '2500000' WHERE key = 'biweekly_salary'", [])?;
        conn.execute("UPDATE settings SET value = 'COP' WHERE key = 'biweekly_salary_currency'", [])?;
        conn.execute("UPDATE settings SET value = 'quincena' WHERE key = 'salary_frequency'", [])?;
        conn.execute("DELETE FROM salary_history", [])?;
        conn.execute(
            "INSERT INTO salary_history (amount, currency, frequency, effective_date) VALUES (?1, 'COP', 'quincena', ?2)",
            params![2500000.0, today],
        )?;

        // 2. Add some recurring expenses (Gastos Fijos) - Total: 1,170,000 monthly
        insert_recurring_expense(&conn, &RecurringExpense {
            id: "test-recur-1".to_string(),
            amount: 1000000.0,
            description: "Arriendo".to_string(),
            frequency: "monthly".to_string(),
            day_of_month: Some(5),
            day_of_week: None,
            category: "Vivienda".to_string(),
        }).ok();

        insert_recurring_expense(&conn, &RecurringExpense {
            id: "test-recur-2".to_string(),
            amount: 120000.0,
        ...
        insert_recurring_expense(&conn, &RecurringExpense {
            id: "test-recur-3".to_string(),
            amount: 50000.0,
            description: "Suscripciones".to_string(),
            frequency: "monthly".to_string(),
        ...
        // 3. Add coherent transactions (Movimientos Variables & Ingresos Extra)
        conn.execute(
            "INSERT INTO view_transactions (id, amount, category, description, date) VALUES (?1, ?2, ?3, ?4, ?5)",
            params!["test-tx-1", -45000.0, "Comida", "Almuerzo Restaurante", today],
        ).ok();
        conn.execute(
            "INSERT INTO view_transactions (id, amount, category, description, date) VALUES (?1, ?2, ?3, ?4, ?5)",
            params!["test-tx-2", -15000.0, "Transporte", "Uber", today],
        ).ok();
        conn.execute(
            "INSERT INTO view_transactions (id, amount, category, description, date) VALUES (?1, ?2, ?3, ?4, ?5)",
            params!["test-tx-3", -250000.0, "Mercado", "Supermercado Semanal", yesterday],
        ).ok();
        conn.execute(
            "INSERT INTO view_transactions (id, amount, category, description, date) VALUES (?1, ?2, ?3, ?4, ?5)",
            params!["test-tx-4", -60000.0, "Entretenimiento", "Cine con amigos", three_days_ago],
        ).ok();
        conn.execute(
            "INSERT INTO view_transactions (id, amount, category, description, date) VALUES (?1, ?2, ?3, ?4, ?5)",
            params!["test-tx-5", 100000.0, "Ingresos", "Venta de artículo usado", five_days_ago],
        ).ok();
    }
